...
```
#### 测试环境如何生成sourcemap并调试
```javascript
// 对于测试环境 如果我们不想sourcemap文件不想和项目一起发布到测试环境上
1. 将devtool设置成false, 不通过devtool选项生成sourcemap文件
2. 使用webpack.SourceMapDevToolPlugin生成sourcemap并向最终打包的文件底部添加我们自定义的sourcemap关联路径
    // new webpack.SourceMapDevToolPlugin({
    //     append: '\n//# sourceMappingURL=http://127.0.0.1:8081/[url]', // 向输出文本里添加的映射文本
    //     filename: '[file].map', // main.js的sourcemap文件叫 main.js.map
    // }),

3. 使用FilemagagerPlugin插件将dist目录下的.map文件拷贝到指定目录中去, 这样发布到测试环境的代码就没有sourcemap文件了
   但是会有sourcemap映射文本的路径，指向我们本地
...
4. 我们想要调试测试环境代码时, 使用http-server在本地将.map文件在8081端口启动服务 开启浏览器的使用sourcemap功能
   这样我们访问测试环境时就能加载我们本地的sourcemap文件了 
```

#### 生产环境如何生成sourcemap文件并调试
```javascript
// 生产环境肯定不能在打包后的文件中添加sourcemap文件的映射

1. 将devtool选项设置为'hidden-source-map'这样打包后的文件中就不会有sourcemap的映射
2. 依旧通过FilemagagerPlugin将生成的sourcemap文件拷贝到指定目录中，不发布到生产环境中
3. 将拷贝的sourcemap文件使用http-server启动服务
4. 访问线上代码, 在开发者工具中打开source面板 => 看到压缩后的代码 => 右键 => Add source map => 将本地的map文件路径粘贴进去就行了
```